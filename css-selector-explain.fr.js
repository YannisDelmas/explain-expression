cssSelectorExplain({
	...
	explanations: {
		...
		pseudo_func: {
			':type': 'name',
			':default': 'de pseudo-classe{{#ref}}{{/ref}} <code>{{&name}}</code><blockquote>avec le paramètre <span class="value">{{args}}</span></blockquote>',
			'lang' : 'qui est dans la langue{{#ref}}{{/ref}} «&nbsp;<span class="value">{{&args}}</span>&nbsp;»',
			'dir': 'dont la langue s’écrit <span class="value">{{args}}</span>{{#ref}} :dir(){{/ref}}',
			'nth-child': 'qui, dans son parent, est un enfant de rang{{#ref}}:nth-child(){{/ref}}&ensp;<span class="value">{{args}}</span>',
			'nth-last-child': 'qui, dans son parent, est un enfant de rang{{#ref}}:nth-last-child(){{/ref}}&ensp;<span class="value">{{args}}</span> en partant de la fin',
			'nth-of-type': 'qui, dans son parent, est de rang{{#ref}}{{/ref}}&ensp;<span class="value">{{args}}</span> parmi les éléments de même balise',
			'nth-last-of-type': 'qui, dans son parent, est de rang{{#ref}}{{/ref}}&ensp;<span class="value">{{args}}</span>, en partant de la fin, parmi les éléments de même balise',
		},
			an_plus_b: '{{a}}</span>×n <span class="make-signed"><span class="value">{{b}}</span>',
			odd: 'impair',
			even: 'pair',
